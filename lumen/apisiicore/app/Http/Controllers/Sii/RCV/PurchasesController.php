...
class PurchasesController extends Controller
{
    ...
    public function diary($day, $month, $year, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $rr  = explode("-", $rut);
            $dd  = $sii->summary($rr[0], $rr[1], $day, $month, $year, "REGISTRO", "COMPRA", $detail);
        ...
        $details = [];
        foreach ($dd['compras']['detalleCompras'] as $vv) {
            if ($vv['fechaEmision'] === $date) {
                $details[] = $vv;
            }
        }
        $dd['compras']['detalleCompras'] = $details;
    ...
    public function montly($month, $year, Request $request): JsonResponse
    {
        ...
        try {
            $sii = new Sii($rut, $pass);
            $rr  = explode("-", $rut);
            $result = $sii->summary($rr[0], $rr[1], null, $month, $year, "REGISTRO", "COMPRA", $detail);
        } catch (\RuntimeException $e) {
            $status = str_contains($e->getMessage(), 'inválidas') ? 401 : 502;
            return response()->json(['error' => $e->getMessage()], $status);
        }
